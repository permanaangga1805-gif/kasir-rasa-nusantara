    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #1a365d 0%, #2b6cb0 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "40px 36px", width: 360, boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 26, color: "#1a365d" }}>KasirKu</div>
          <div style={{ color: "#718096", fontSize: 13 }}>Sistem Kasir Digital</div>
        </div>
...
function ReceiptModal({ order, settings, onClose }) {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      <html><head><title>Struk #${String(order.id).slice(-6)}</title>
      <style>
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: 380, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
        <div ref={ref}>
          <div className="center" style={{ textAlign: "center", borderBottom: "2px dashed #e2e8f0", paddingBottom: 14, marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 11, color: "#a0aec0" }}>No: #{String(order.id).slice(-6)}</div>
...
export default function KasirApp() {
  ...
  const [newProduct, setNewProduct] = useState({ name: "", price: "", category: "Makanan", icon: "🛒", stock: "", minStock: "10" });
  ...
  const handlePay = () => {
    ...
    const order = {
      id: Date.now(), items: [...cart], subtotal, discount, discAmt, taxAmt, total,
  ...
  const saveProduct = () => {
    ...
    setEditProduct(null);
    setNewProduct({ name: "", price: "", category: "Makanan", icon: "🛒", stock: "", minStock: "10" });
  };
  ...
  const exportExcel = () => {
    ...
    const txRows = orders.map(o => ({
      "No. Struk": "#" + String(o.id).slice(-6), "Tanggal": o.date, "Kasir": o.cashierName || "-",
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(expRows), "Keuangan");
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(stockRows), "Stok");
    ...
    XLSX.writeFile(wb, `Laporan_KasirKu_${new Date().toLocaleDateString("id-ID").replace(/\//g,"-")}.xlsx`);
      ...
      <div style={{ background: "#1a365d", color: "#fff", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 12px rgba(0,0,0,.2)", flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, opacity: 0.7 }}>Sistem Kasir Digital</div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: 20, marginBottom: 20, border: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
              ...
              {editProduct && <button onClick={() => { setEditProduct(null); setNewProduct({ name: "", price: "", category: "Makanan", icon: "🛒", stock: "", minStock: "10" }); }} style={{ ...S.btn, background: "#e2e8f0", color: "#4a5568" }}>Batal</button>}
            </div>
          </div>
              ...
              <thead>
                <tr style={{ background: "#ebf8ff", color: "#2c5282" }}>
                  {["Produk","Kategori","Harga","Stok","Min. Stok","Status","Aksi"].map(h => (
                    <th key={h} style={{ padding: "11px 14px", textAlign: "left", fontSize: 13, fontWeight: 700 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: 24 }}>
          ...
          {orders.length === 0 ? (
            ...
            <div key={o.id} style={{ background: "#fff", borderRadius: 12, padding: "14px 18px", marginBottom: 10, border: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#1a365d" }}>#{String(o.id).slice(-6)}</div>
        ...
        <div style={{ maxWidth: 620, margin: "0 auto", padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: 24, border: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
              ...
              <button onClick={() => { if (window.confirm("Hapus semua data? Ini tidak bisa diurungkan!")) { setOrders([]); setExpenses([]); setProducts(INITIAL_PRODUCTS); showToast("Data direset!", "error"); }}} style={{ ...S.btn, background: "#fff5f5", color: "#e53e3e" }}>🗑️ Reset Semua Data</button>
            </div>
